...
export const About: React.FC = () => {
  ...
  return (
    <section id="about" className="py-24 bg-[#08080a] relative overflow-hidden border-t border-white/10">
      {/* Glow Effects */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center space-x-2 bg-red-500/10 border border-red-500/20 px-3.5 py-1.5 rounded-full text-xs text-red-400 font-semibold tracking-wider uppercase">
        ...
        {activeTab === 'timeline' && (
          <div className="relative border-l-2 border-red-600/40 ml-4 sm:ml-32 space-y-10 my-8">
            {MILESTONES.map((item, idx) => (
              <div key={item.year} className="relative pl-8 sm:pl-12 group">
                {/* Timeline Dot */}
                <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-[#08080a] border-2 border-red-500 group-hover:scale-125 group-hover:bg-red-500 transition-all shadow-md shadow-red-500/50" />

                {/* Year Badge */}
                <div className="hidden sm:block absolute -left-32 top-0 text-right w-24">
                  <span className="text-xl font-black text-red-500 font-mono">{item.year}</span>
                ...
                <div className="glass-panel glass-panel-hover p-6 rounded-2xl shadow-xl">
                  <div className="sm:hidden text-red-400 font-mono font-bold text-sm mb-1">{item.year} • {item.category}</div>
                  <h3 className="text-xl font-bold text-white mb-2">{item.title}</h3>
                  <p className="text-sm text-slate-300 leading-relaxed mb-4">{item.description}</p>
                  <div className="inline-flex items-center space-x-2 bg-red-500/10 px-3 py-1 rounded-md text-xs font-mono text-rose-300 border border-red-500/20">
        ...
        {activeTab === 'achievements' && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 my-8">
            <div className="glass-panel p-6 rounded-2xl text-center space-y-2">
              <div className="text-3xl font-black text-red-500 font-mono">$42 Billion+</div>
            ...
            <div className="glass-panel p-6 rounded-2xl text-center space-y-2">
              <div className="text-3xl font-black text-rose-400 font-mono">140+</div>
            ...
            <div className="glass-panel p-6 rounded-2xl text-center space-y-2">
              <div className="text-3xl font-black text-amber-500 font-mono">18,000+</div>
