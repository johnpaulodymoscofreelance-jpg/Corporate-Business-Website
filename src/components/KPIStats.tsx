...
export const KPIStats: React.FC = () => {
  const kpis = [
    { label: 'Managed Client Enterprise Assets', value: '$42B+', icon: <DollarSign className="w-5 h-5 text-red-400" /> },
    { label: 'Countries & Global Regions', value: '140+', icon: <Globe className="w-5 h-5 text-rose-400" /> },
    { label: 'Enterprise Transformation Projects', value: '3,800+', icon: <Briefcase className="w-5 h-5 text-amber-400" /> },
    { label: 'System SLA Availability Guarantee', value: '99.999%', icon: <ShieldCheck className="w-5 h-5 text-emerald-400" /> },
    { label: 'Global Systems Engineers & Scientists', value: '18,000+', icon: <Users className="w-5 h-5 text-red-500" /> },
    { label: 'Customer Satisfaction Index (CSAT)', value: '98.8%', icon: <Award className="w-5 h-5 text-amber-400" /> },
  ];

  return (
    <section className="py-20 bg-[#08080a] border-t border-b border-white/10 relative">
